...
const useCartStore = create<ICartState>()(
  persist(
    (set, get) => ({
      ...
      addToSaveForLater: (cartItem: ICartItem) =>
        set((state) => {
          const doExist = state.saveForLaterCart.items.find(
          ...
          return {
            saveForLaterCart: {
              ...
              items: [...state.saveForLaterCart.items, cartItem],
            },
            cart: { ...state.cart, items: updatedCartItems },
          };
        }),
      removeFromSaveForLater: (productId: string) =>
        set((state) => ({
          saveForLaterCart: {
            ...state.saveForLaterCart,
            items: state.saveForLaterCart.items.filter(
              (item: ICartItem) => item.product.id !== productId,
            ),
          },
        })),
      clearSaveForLater: () => set({ saveForLaterCart: emptyCart as ICart }),
      moveToCart: (cartItem: ICartItem) =>
        set((state) => {
          const existingItem = state.saveForLaterCart.items.find(
            (item: ICartItem) => item.product.id === cartItem.product.id,
          );
          if (!existingItem) {
